...
func GetPublicKey(c *app.RequestContext) ([]byte, error) {
	var bytePublicKey []byte
	// get PublicKey URL
	publicKeyURLBase64 := c.GetHeader("x-oss-pub-key-url")
	if string(publicKeyURLBase64) == "" {
	...
	publicKeyURL, _ := base64.StdEncoding.DecodeString(string(publicKeyURLBase64))
...
func GetAuthorization(ctx *app.RequestContext) ([]byte, error) {
	var byteAuthorization []byte
	// Get Authorization bytes : decode from Base64String
	strAuthorizationBase64 := ctx.GetHeader("authorization")
	if string(strAuthorizationBase64) == "" {
	...
	byteAuthorization, _ = base64.StdEncoding.DecodeString(string(strAuthorizationBase64))
	return byteAuthorization, nil
}

// GetMD5FromNewAuthString : Get MD5 bytes from Newly Constructed Authorization String.
func GetMD5FromNewAuthString(ctx *app.RequestContext) ([]byte, string, error) {
	var byteMD5 []byte
	// Construct the New Auth String from URI+Query+Body

	body, err := ioutil.ReadAll(ctx.Request.BodyStream())
	if err != nil {
	...
	strAuth := ""
	if string(ctx.Request.URI().QueryString()) == "" {
		strAuth = fmt.Sprintf("%s\n%s", strURLPathDecode, strCallbackBody)
	} else {
		strAuth = fmt.Sprintf("%s?%s\n%s", strURLPathDecode, ctx.Request.URI().QueryString(), strCallbackBody)
	}
...
func ResponseFailed(ctx *app.RequestContext) {
